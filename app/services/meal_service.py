...
class MealService:
    ...
    def __init__(self):
        ...
        self.previous_meals: Dict[
            str, Meal
        ] = {}  # последнее сгенерированное блюдо для каждого индекса
        self.meal_history: Dict[
            str, List[Meal]
        ] = {}  # история блюд для каждого индекса
        self.session_history: List[
            Meal
        ] = []  # общая история всех сгенерированных блюд за сессию

    def generate_meal_plan(
        self, available_ingredients: Optional[List[str]] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        for meal_type in meal_types:
            ...
            for attempt in range(max_attempts):
                ...
                if not validation_result.is_valid:
                    logger.error(
                        f"Ошибка валидации рецепта: {validation_result.get_errors()}"
                    )
                    continue
            ...
            if not unique_meal_found and last_valid_meal:
                logger.warning(
                    f"Не удалось сгенерировать уникальный рецепт, используем последний валидный: {last_valid_meal.name}"
                )
