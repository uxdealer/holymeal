...
class MealValidator:
    ...
    def validate_meal_data(data: Dict[str, Any]) -> ValidationResult:
        ...
        required_fields = ['name', 'ingredients', 'instructions', 
                         'cooking_time', 'calories_per_serving']
            ...
            if not isinstance(data['ingredients'], list):
                ...
                for i, ing in enumerate(data['ingredients']):
                    if not isinstance(ing, str):
                        result.add_error('ingredients', 
                            f"Ингредиент {i+1} должен быть строкой")
                    elif ' - ' not in ing:
                        result.add_error('ingredients',
            ...
            if not isinstance(data['instructions'], list):
                ...
                for i, step in enumerate(data['instructions']):
                    if not isinstance(step, str):
                        result.add_error('instructions',
                            f"Шаг {i+1} должен быть строкой")
                    elif len(step.strip()) == 0:
                        result.add_error('instructions',
                            f"Шаг {i+1} не может быть пустым")
        ...
        if 'calories_per_serving' in data:
            try:
                ...
                if calories <= 0:
                    result.add_error('calories_per_serving',
